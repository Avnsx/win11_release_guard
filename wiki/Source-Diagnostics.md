...
| Active baseline notice says security classification is unavailable. | Notice fields `security_evidence_source: unavailable` and `support_article_validation_status: unavailable` with no `source_url`, and no `support_article_enrichment_*` or `msrc_cvrf_*` events. | Microsoft's Update History Atom feed has not published the baseline KB entry yet (observed lagging Patch Tuesday by more than two weeks in July 2026). With no Atom link there is no Support URL or MSRC month to check, so no enrichment fetch is attempted and the neutral wording is honest no-data output, not a parser error. Classification appears automatically once Microsoft publishes the Atom entry and a later publish run picks it up. |
